...
Generates a mock CSV (12 students × 5 attempts × 3 criteria) and a set of PNG plots
matching the INSIGHTs Sim-U prototype description.
...
# New schema: first four Professional Integrity criteria, each with 4 elements
...
GROUPS = {
    ...
    "PI_04_Maintain_Confidentiality": [
        "confidentiality_agreements",
        "learner_data_protection",
        "scenario_content_security",
...
def generate_mock_data(students, attempts, seed=RANDOM_SEED, groups=GROUPS):
    ...
    scenarios = ["SIM-U : Cardio A", "SIM-U : Cardio B", "SIM-U : Respiratory"]
...
def plot_simu_x_socratic_network(simudf, socratic_wide_df, out_path, miss_threshold=70):
    """Create a bipartite-like network showing correlations between Sim-U elements and Socratic metrics.

    We'll compute Pearson correlations between each Sim-U element (averaged per student) and each socratic metric.
    ...
    plt.title('Sim-U elements × Socratic metrics (|corr| >= 0.3)')
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(out_path, dpi=DPI)
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(description='Generate INSIGHTs Sim-U prototype CSV and PNGs')
    ...
    # Create cross-domain network between Sim-U elements and Socratic metrics
